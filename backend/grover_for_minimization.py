...
def load_values_in_arr(circuit: QuantumCircuit, n_ancillary_qubits: int, n_data_qubits: int, arr: List[int]):
    ...
    :param arr: List of ints:
        The list we are searching.
    :return: QuantumCircuit:
        The provided circuit, except with added gates to conditionally load all the values in arr onto the
         ancillary register
    """
...
def minimization_circuit(arr: List[int], x: int) -> QuantumCircuit:
    """
    Build the complete quantum minimization circuit to find if there exists an element in arr < x.
    ...
    # We need one wire in the data register for each element of arr.
    number_of_data_qubits_required = len(arr)
    quantum_data_register = QuantumRegister(number_of_data_qubits_required, 'data_q')

    # Measurment results from the quantum data register will be projected onto classical wires.
    classical_data_register = ClassicalRegister(number_of_data_qubits_required, "data_c")

    # We need enough ancillary qubits to represent 0 -> sum(arr) or x, whichever is larger.
    #  Recall you can represent up to 2^n - 1 with n bits.
    number_ancillary_qubits_required = max(math.ceil(math.log(sum(arr) + 1, 2)), math.ceil(math.log(x + 1, 2)))
    quantum_ancillary_register = QuantumRegister(number_ancillary_qubits_required, 'ancillary_q')
    # Ancillary bits are just work bits, we won't need to measure them.

    # Create a quantum circuit
    circuit = QuantumCircuit(quantum_ancillary_register, quantum_data_register, classical_data_register)
